...
export default function ProductTemplatesPage() {
  ...
  return <div className="p-8">
    <div className="mb-8"><h1 className="text-2xl font-bold text-zinc-100 font-mono">Product Templates</h1><p className="text-zinc-500 text-sm mt-1 font-mono">Preset bundles and validation rules attached to each product.</p></div>
    <div className="flex gap-6 h-[calc(100vh-200px)]">
      <div className="w-52 shrink-0 flex flex-col gap-1">
        ...
        <Tabs orientation="vertical" value={selectedId} onValueChange={(id) => { const template = templates.find((item) => item.id === id); if (template) select(template) }}>
          <TabsList className="w-52 flex-col items-stretch bg-transparent p-0 h-fit">{templates.map((template) => <TabsTrigger key={template.id} value={template.id} className="justify-start rounded-none border-transparent px-3 py-2 text-sm font-mono text-zinc-500 transition-colors hover:bg-zinc-800/40 hover:text-zinc-300 data-active:border-l-2 data-active:border-violet-500 data-active:bg-zinc-800/60 data-active:text-zinc-100 data-active:shadow-none">{template.name}</TabsTrigger>)}</TabsList>
        </Tabs>
      </div>
      {selected ? <div className="flex-1 overflow-y-auto pr-2">
        <Card className="bg-zinc-900/40 p-6 ring-zinc-800 mb-4">
