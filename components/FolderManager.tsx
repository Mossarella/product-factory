    ...
    <div className="font-mono">
      <div className="flex flex-wrap gap-2">
        {folders.map((folder, index) => (
          <Badge key={`${folder}-${index}`} variant="outline" className="h-auto rounded-none border-zinc-700 bg-zinc-900 px-2 py-0.5 font-normal text-zinc-300">
            <span>{folder}</span>
      ...
      <form className="mt-3 flex gap-2" onSubmit={addFolder}>
        <Input
          ...
          className="h-auto rounded-none border-zinc-700 bg-zinc-900 px-3 py-1.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-violet-500 focus:outline-none focus-visible:ring-0"
        />
        <Button type="submit" variant="outline" className="h-auto rounded-none border-zinc-700 bg-zinc-800 px-3 py-1.5 font-normal text-zinc-300 hover:bg-zinc-700">Add</Button>
      </form>
    </div>
  )
}
