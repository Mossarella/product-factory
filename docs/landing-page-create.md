# Spec: Create Landing Page at /

## Overview
Replace `app/page.tsx` with a marketing landing page for Product Factory.
The factory UI is now at `/app`. This landing page lives at `/`.

## Stack
- Next.js App Router, TypeScript, Tailwind CSS v4
- Dark monospace theme: bg-zinc-950, text-zinc-100, accent violet-500/violet-600
- Font: Geist Mono (already set on body via layout.tsx)
- NO shadcn, NO custom CSS — only Tailwind utility classes
- This is a client component ('use client') NOT needed — make it a server component (no 'use client')

## File to create
`/Users/Noppheera.Bha/Desktop/Work/product-factory/app/page.tsx`
      ...
      <nav className="border-b border-zinc-800 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src="/api/slot/logo" alt="" className="w-7 h-7 object-contain rounded" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
...
- The img tag in nav has an inline onError handler — that requires 'use client' OR extract the nav into a small client component. To avoid this: either remove the onError, or just omit the logo img entirely from the nav since it's a landing page. OMIT the logo img from nav to keep it a server component.
- Use `Link` from `next/link` for internal links
- `/api/buy` is an internal API route that redirects to Stripe
- Keep Tailwind v4 style: no `tailwind.config.js` needed, just utility classes

## Verification
- File exists at correct path
- No 'use client' directive
- Imports only next/link
- Print: === COMPLETE: landing page created ===
