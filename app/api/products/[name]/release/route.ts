...
async function extractReleaseArtifacts(buffer: Buffer): Promise<ReleaseArtifacts | null> {
  const zip = await JSZip.loadAsync(buffer)
  const listingFile = zip.file('etsy-listing.json')
  const summaryFile = zip.file('release-summary.json')
  if (!listingFile || !summaryFile) return null
  try {
    return {
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  const artifacts = await extractReleaseArtifacts(buffer).catch(() => null)
  if (!artifacts) return NextResponse.json({ error: 'Build does not contain release artifacts', code: 'RELEASE_ARTIFACTS_MISSING' }, { status: 422 })
